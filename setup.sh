...
readonly VENV_SUB_DIR="aver"
readonly PYCUDA_VERSION="2025.1.2" 
readonly OPENCV_VERSION="4.10.0.84"
readonly PYCUDA_INDEX_URL="https://pypi.jetson-ai-lab.io/jp6/cu126" 
...
DESCRIPTION:
    ...
    ${BULLET_SYMBOL} Create a Python virtual environment: ~/aver/realsense_env
...
# Helper: Get log target (/dev/null or log file)
get_log_target() {
...
# Function to run a command, log output, and only print error on failure
run_command_log_error() {
    ...
    # Execute the command, redirecting stdout/stderr to the determined log target.
    # >> $log_target 2>&1 ensures all output is either saved to log or discarded to /dev/null.
...
# Check if a Python package is installed and has the correct version in VENV
check_installed_package() {
    ...
    # Run pip show within VENV, redirecting output to log target
    ...
    if [ -z "$required_version" ] || [ "$installed_version" == "$required_version" ]; then
        print_success "%s (Version %s) is already installed and matches the required version. Skipping installation." "$package_name" "$installed_version"
        return 0 
    else
...
check_jetpack_version() {
    ...
    if [ "$l4t_release" -lt 36 ]; then
        ...
        print_warning "The pycuda wheel ($PYCUDA_INDEX_URL) is compiled for JetPack 6 (R36.x)."
...
check_venv_status() {
    ...
    if [ -d "$VENV_PATH" ]; then
        ...
        # Ask user if they want to recreate VENV (default: n)
        if ask_user "Do you want to re-CREATE the virtual environment (This will DELETE the existing one)?" "n"; then
            ...
            rm -rf "$VENV_PATH"
            VENV_CREATED=false # Needs recreation
            return 0
        else
            VENV_CREATED=true # Use existing VENV
...
install_system_deps() {
    # This is the only step that requires sudo. Commands are combined with '&&' 
    # to ensure atomic execution and error checking within the 'eval' call.
    run_command_log_error "Installing system dependencies (python3-pip, python3-venv)" \
...
create_and_activate_venv() {
    local log_target=$(get_log_target)
    # 1. Create venv (run as current user)
    ...
    # 2. Upgrade pip, setuptools, wheel (run as current user)
    local pip_command="$VENV_PATH/bin/pip"
...
install_pycuda() {
    # Check for pycuda 2025.1.2
    if check_installed_package pycuda "$PYCUDA_VERSION"; then
        PYCUDA_INSTALLED=true
        return 0
    fi
    
    print_info "Installing pycuda $PYCUDA_VERSION from $PYCUDA_INDEX_URL"
    
    if run_command_log_error "Downloading and installing pycuda $PYCUDA_VERSION" \
        "$VENV_PATH/bin/pip" install "pycuda==$PYCUDA_VERSION" -f "$PYCUDA_INDEX_URL" --no-cache-dir; then
...
execute_model_script() {
    # Ask if user wants to run the external model script (default: y)
    ...
    if ask_user "Do you want to run the model download and conversion script (${BOLD}$MODEL_SCRIPT_PATH${NC})?" "y"; then
        ...
        if ! /bin/bash "$MODEL_SCRIPT_PATH"; then
...
if ! check_venv_status; then exit 1; fi # Check VENV existence and ask for recreation
...
printf "  ${BOLD}1. Activate the Environment:${NC}\n"
printf "     In a new terminal or after logout/login, run:\n"
